...
public class FoodService {
    ...
    private static final List<String> KCAL_KEYS = List.of("enerc", "kcal", "NUTR_CONT1", "에너지(kcal)", "에너지(KCAL)", "에너지kcal");
    ...
    public List<FoodDTO> searchSimple(String name, int page, int perPage) {
        ...
        try {
            ...
            if (records.isArray() && records.size() > 0) {
                for (JsonNode n : records) addIfPresent(dedup, n);
            } else {
                JsonNode data = root.path("data");
                if (data.isArray() && data.size() > 0) {
                    for (JsonNode n : data) addIfPresent(dedup, n);
                } else {
                    JsonNode items = root.path("response").path("body").path("items").path("item");
                    if (items.isArray()) {
                        for (JsonNode n : items) addIfPresent(dedup, n);
                    } else if (items.isObject()) {
                        addIfPresent(dedup, items);
                    } else {
                        for (JsonNode n : findParentsAny(root, NAME_KEYS)) addIfPresent(dedup, n);
                        for (JsonNode n : findParentsAny(root, KCAL_KEYS)) addIfPresent(dedup, n);
                    }
                }
            }
        ...
        for (String k : keys) {
            ...
            try {
                ...
                for (char c : raw.toCharArray()) {
                    ...
                    else if ((c == '+' || c == '-') && !sign && sb.length() == 0) { sign = true; sb.append(c); }
                    else if (sb.length() > 0) break;
                }
                if (sb.length() > 0) return Double.valueOf(sb.toString());
    ...
    private static Collection<JsonNode> findParentsAny(JsonNode root, List<String> keys) {
        ...
        for (String k : keys) {
            ...
            if (found != null) set.addAll(found);
        }
        return set;
    }
}
